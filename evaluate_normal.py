...
prompt = prompt_qwen if model_path.startswith('qwen') else prompt_phi if model_path.startswith('phi') else prompt_llama
...
for question, answer in test_dataset:
    ...
    # for i in tqdm(range(10)):
        ...
        # print(input)
        output = model.generate(tokenizer(input, return_tensors='pt').input_ids.to('cuda'), early_stopping=True, max_new_tokens=1024)
        ...
        if "The answer is:" in ans:
            ...
            try:
                ans = ans[ans.rindex(r'\boxed{') + 8:ans.rindex('}')]
                if ans == '':
                ...
                print(ans)
                prompt, _ = generate_prompt(test_dataset[question][i], test_dataset[answer][i],i)
        ...
        if i % 100 == 0 or i == len(test_dataset) -1 or i == 9: 
